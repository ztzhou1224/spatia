...
cargo run -p spatia_cli -- geocode ./spatia.duckdb "San Francisco, CA"
...
| Module | Purpose |
|---|---|
| `executor` | `execute_command(cmd: &str)` — parses and dispatches string commands (`ingest`, `schema`, `geocode`, `overture_extract`, `overture_search`, `overture_geocode`) |
...
| `geocode` | `geocode_batch_hybrid` — cache-first geocoding with Geocodio fallback |
| `geocode_cache` | `ensure_cache_table` / `cache_lookup` / `cache_store` — DuckDB-backed `geocode_cache` table |
| `geocodio` | `geocode_via_geocodio` — Geocodio REST API batch client |
...
geocode <db_path> <address_1> <address_2> ...                    → JSON array of GeocodeResult
...
`geocode_batch_hybrid` implements a cache-first strategy:
1. Check `geocode_cache` in DuckDB — return cached coordinates for matching addresses.
2. For remaining addresses, call the Geocodio batch endpoint.
3. Write all newly resolved results to `geocode_cache`.

Geocodio env vars: `SPATIA_GEOCODIO_API_KEY` (required), `SPATIA_GEOCODIO_BATCH_SIZE` (default 100).
