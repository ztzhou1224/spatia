...
pub fn geocode_batch_hybrid(addresses: &[String]) -> EngineResult<Vec<GeocodeResult>> {
    let mode = geocode_mode_from_env()?;
    match mode {
        GeocodeMode::OneShot => geocode_batch_oneshot(addresses),
        GeocodeMode::Daemon => geocode_batch_daemon(DEFAULT_GEOCODER_URL, addresses),
        GeocodeMode::Auto => {
            if should_use_daemon(addresses.len()) {
                geocode_batch_daemon(DEFAULT_GEOCODER_URL, addresses)
            } else {
                match geocode_batch_oneshot(addresses) {
                    Ok(results) => Ok(results),
                    Err(_) => geocode_batch_daemon(DEFAULT_GEOCODER_URL, addresses),
                }
            }
        }
    }
